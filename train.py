...
def CNN():
	...
	batched = tf.contrib.layers.batch_norm(dense_layer_2,center=True, scale=True, is_training = training_phase)


	logit_layer = tf.layers.dense(inputs=batched, units=k)
...
with tf.Session() as sess:
	...
	#VISUALIZE CONV 1 FILTERS
	kernel = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'conv1/kernel')[0]
	layer_1_weights = sess.run(kernel)
	f = open("tmp/conv1_filters.pkl",'wb')
	pickle.dump(layer_1_weights, f)
	f.close()



	#FOOLNG THE NETWORK
	fool_file = open("tmp/fooling", 'w')
	for num_pixels_to_change in range(1,100):
		fooled_X = []
		for val_x in X_val:		
			#val_x is modified to get fool_x
			fool_x = np.array(val_x)
			#Make mask 	
			mask = np.zeros(784, dtype=int)
			mask[:num_pixels_to_change] = 1
			np.random.shuffle(mask)
			mask = mask.astype(bool)
			#
			r = np.random.rand(784)*np.max(val_x)
			fool_x[mask] = r[mask]
			fooled_X.append(fool_x)
		fooled_X = np.array(fooled_X)
		#print(np.sum(np.sum(fooled_X)) - np.sum(np.sum(X_val)))

		#preds = sess.run(last_layer, feed_dict={X_ph: fooled_X[:10], Y_ph: Y_val[:10], training_phase: False})
		#preds = np.argmax(preds, axis=1)
		#print(preds, np.argmax(Y_val, axis=1))
		loss_val, acc_val = sess.run([cost, accuracy], feed_dict={X_ph: fooled_X, Y_ph: Y_val, training_phase: False})
		fool_file.write(str(num_pixels_to_change) + " " + str(acc_val) + "\n")
		#print(str(num_pixels_to_change) + " " + str(acc_val))
	fool_file.close()



# 	# #If testing required, get predictions
# 	# predictions = sess.run(last_layer, feed_dict={X_ph: X_test, training_phase: False})
